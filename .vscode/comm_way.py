...
class Way():
      ...
      def log_write_way(self,testcode,data):
            today = datetime.datetime.now().strftime('%Y-%m-%d %H')
            nowtime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            log=open(r'./log/%s.txt' % today,mode='a')
            log.writelines('{++++++++++++++++++++++%s++++++++++++++++++++++}' % testcode)
            log.writelines('\n')
            log.writelines(data)
            log.writelines('\n')
            log.writelines("{'当前时间':'%s'}" % nowtime)
            log.writelines('\n')
            log.writelines('\n')
            log.close()
      def log_read_way(self,name):
            log=open(r'./log/%s.txt' % name)
            log_read=log.read()
            log.close()
            return log_read
